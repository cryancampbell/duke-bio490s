...
```{r}
plot(hypoxiaTested$value_2,hypoxiaTested$value_1, xlim = c(0,100), ylim = c(0,100), col = "grey30", ylab = "FKPM, Hypoxic Condition", xlab = "FKPM, Normal Condition")
points(hypoxiaTIMM$value_2,hypoxiaTIMM$value_1, col = "green", pch = 19)

```

volcano plot
log(pvalue) v. fold change
```{r}
plot(x = hypoxiaTested$log2.fold_change., y = -log10(hypoxiaTested$p_value)  )
abline(h = 2)
points(hypoxiaTIMM$log2.fold_change., -log10(hypoxiaTIMM$p_value), col = "green", pch = 19)
...
analyze OR genes
```{r}
#code to analyze only OR here
hypoxiaOR <- hypoxiaTested[hypoxiaTested$gene_family == "OR",]
```
